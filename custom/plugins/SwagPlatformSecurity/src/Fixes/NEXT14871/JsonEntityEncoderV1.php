...
class JsonEntityEncoderV1 extends CoreJsonEntityEncoder
{
    /**
     * @var Serializer
     */
    ...
    private function removeNotAllowedFields(array $decoded, EntityDefinition $definition, string $baseUrl, int $apiVersion): array
    {
        ...
        foreach ($decoded as $key => &$value) {
            if ($key === 'extensions') {
                $decoded[$key] = $this->removeNotAllowedFields($value, $definition, $baseUrl, $apiVersion);
            ...
            if ($field instanceof AssociationField) {
                ...
                if ($field instanceof ManyToManyAssociationField || $field instanceof OneToManyAssociationField) {
                    ...
                    foreach ($value as $id => $entity) {
                        $value[$id] = $this->removeNotAllowedFields($entity, $referenceDefinition, $baseUrl, $apiVersion);
                    }
                }
            }
        }
