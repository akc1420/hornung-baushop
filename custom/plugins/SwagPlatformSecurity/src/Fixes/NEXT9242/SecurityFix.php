...
class SecurityFix extends AbstractSecurityFix
{
    private const API_ROUTES = [
        ...
        'api.user.create',
        'api.user.update',
        'api.custom.store.logout',
        'api.custom.store.login',
    ];

    private const ADMIN_SCOPE_API_ROUTES = [
        'api.custom.store.logout',
        'api.custom.store.login',
    ];

    private const USER_SCOPE_API_ROUTES = [
        'api.user.delete',
        'api.user.create',
        'api.user.update',
    ];
    ...
    public function onValidateApiRequest(ControllerEvent $event): void
    {
        ...
        if (
            in_array($request->attributes->get('_route'), self::USER_SCOPE_API_ROUTES, true) &&
            !$this->hasScope($request, UserVerifiedScope::IDENTIFIER)
        ) {
        ...
        if (
            in_array($request->attributes->get('_route'), self::ADMIN_SCOPE_API_ROUTES, true) &&
            !$this->hasScope($request, AdminScope::IDENTIFIER)
        ) {
            throw new AccessDeniedHttpException(sprintf('This access token does not have the scope "%s" to process this Request', AdminScope::IDENTIFIER));
        }
