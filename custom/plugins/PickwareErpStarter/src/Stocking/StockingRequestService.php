...
class StockingRequestService
{
    ...
    public function createStockingRequestForGoodsReceipt(string $goodsReceiptId, Context $context): StockingRequest
    {
        $criteria = new Criteria([$goodsReceiptId]);
        $criteria
            ...
            ->addFilter(new NotFilter(
                ...
                [
                    new EqualsFilter('productId', null),
                    new EqualsFilter('quantity', 0),
                ],
            ));

        /** @var GoodsReceiptEntity $goodsReceipt */
        $goodsReceipt = $this->entityManager->getOneBy(
            GoodsReceiptDefinition::class,
            $criteria,
            $context,
        );

        if (!$goodsReceipt->getWarehouseId()) {
            throw new InvalidArgumentException(
                'A stocking request cannot be created for a goods receipt for which the warehouse was deleted!',
            );
        }

        $productQuantities = array_map(
            fn (GoodsReceiptItemEntity $goodsReceiptItem) => new ProductQuantity(
                $goodsReceiptItem->getProductId(),
                $goodsReceiptItem->getQuantity(),
            ),
            $goodsReceipt->getItems()->getElements(),
        );

        return new StockingRequest($productQuantities, $goodsReceipt->getWarehouseId());
    }
}
