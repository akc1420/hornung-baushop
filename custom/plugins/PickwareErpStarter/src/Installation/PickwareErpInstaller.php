...
class PickwareErpInstaller
{
    ...
    {
        $this->postUpdate($installContext);
    }

    public function postUpdate(InstallContext $installContext): void
    {
        (new UpsertLocationTypesInstallationStep($this->db))->install();
        $this->mailTemplateInstaller->installMailTemplate(new ReorderMailTemplate(), $this->businessEventsLogger, $installContext->getContext());
        (new CreateReorderNotificationFlowInstallationStep($this->entityManager))->install();
        (new CreateTransitionOrderToDoneAfterShippingFlowInstallationStep($this->entityManager))->install();
        (new UpsertSpecialStockLocationsInstallationStep($this->db))->install();
        (new CreateInitialWarehouseInstallationStep($this->db, $this->entityManager))->install();
        ...
        $this->stateMachineInstaller->ensureStateMachine(new GoodsReceiptStateMachine());
        (new UpsertProductSupplierConfigurationsInstallationStep($this->db))->install();
        $this->mailTemplateInstaller->installMailTemplate(new SupplierOrderMailTemplate(), $this->businessEventsLogger, $installContext->getContext());
        ...
        $this->numberRangeInstaller->ensureNumberRange(new GoodsReceiptNumberRange());
