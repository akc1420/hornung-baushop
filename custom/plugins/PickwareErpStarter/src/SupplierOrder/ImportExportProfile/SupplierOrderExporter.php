...
class SupplierOrderExporter implements Exporter, FileExporter, HeaderExporter
{
    ...
    public function exportChunk(string $exportId, int $nextRowNumberToWrite, Context $context): ?int
    {
        ...
        $exportRows = $this->getSupplierOrderExportRows($criteria, $exportConfig['locale'], $this->getColumns($exportConfig), $context);
    ...
    public function getHeader(string $exportId, Context $context): array
    {
        ...
        $translatedColumns = array_map(
            ...
            $this->getColumns($export->getConfig()),
        );
    ...
    private function getColumns(array $exportConfig): array
    {
        ...
        return $exportConfig['columns'] ?? ($systemConfigColumnIdentifiers ? $systemConfigColumns : null) ?? self::DEFAULT_COLUMNS;
    }
}
