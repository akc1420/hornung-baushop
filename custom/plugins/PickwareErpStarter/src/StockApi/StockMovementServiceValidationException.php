...
class StockMovementServiceValidationException extends Exception implements JsonApiErrorSerializable
{
    ...
    public static function operationMovesStockToBinLocationsForNotStockManagedProducts(array $productIds): self
    {
        return new self(new JsonApiError([
            ...
            'title' => 'Operation moves stock vor not stock managed products to bin locations',
            'detail' => sprintf(
                'The attempted operation was aborted because it would move stock for not stock managed products into bin locations. Product ids: %s.',
                implode(', ', $productIds),
            ),
            'meta' => [
                'productIds' => $productIds,
