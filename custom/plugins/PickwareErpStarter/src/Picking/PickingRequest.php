 ...
 * @extends Collection<ProductPickingRequest>
 */
class PickingRequest extends Collection
{
     ...
     */
    public function createProductQuantityLocations(): array
    {
        ...
        foreach ($this->elements as $productPickingRequest) {
            foreach ($productPickingRequest->getPickLocations() as $pickLocation) {
                ...
                $productQuantityLocations[] = new ProductQuantityLocation(
                    $pickLocation->getStockLocationReference(),
                    $productPickingRequest->getProductId(),
                    $pickLocation->getQuantityToPick(),
                );
            }
        }
    ...
    /**
     * Returns the quantities of the products that have to be picked
     *
     * @return ImmutableCollection<ProductQuantity>
     */
    public function getProductQuantities(): ImmutableCollection
    {
        return (new ImmutableCollection($this->elements))
            ->map(
                fn(ProductPickingRequest $pickingRequest) => new ProductQuantity(
                    $pickingRequest->getProductId(),
                    $pickingRequest->getQuantity(),
                ),
            );
    }
