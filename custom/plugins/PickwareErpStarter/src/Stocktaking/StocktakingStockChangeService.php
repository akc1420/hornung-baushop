...
class StocktakingStockChangeService
{
    ...
    private function snapshotCountingProcessItemsAndCreateImportExportElements(string $stocktakeId, string $importId): void
    {
        ...
        // Create import export elements for counting processes in bin locations. The row json body must match the
        ...
        $this->connection->executeStatement(
            'INSERT INTO `pickware_erp_import_export_element` (
                ...
                JSON_OBJECT(
                    ...
                    \'bin location\', binLocation.`code`,
                    \'change\', snapshotCountingProcessItem.`stock_difference`
                ),
                NOW()
            ...
            -- Only consider counting processes on bin locations
            AND binLocation.`id` IS NOT NULL;',
            [
                'stocktakeId' => hex2bin($stocktakeId),
        ...
        $maxRowNumber = $this->connection->fetchFirstColumn(
        ...
        // Create import export elements for counting processes in the unknown stock location. This aggregation differs
        // from counting processes of bin locations. The row json body must match the "relative stock change import" csv
        // format.
        $this->connection->executeStatement(
            'INSERT INTO `pickware_erp_import_export_element` (
                `id`,
                ...
                JSON_OBJECT(
                    \'product number\', product.`product_number`,
                    \'warehouse code\', warehouse.`code`,
                    \'bin location\', :binLocationCodeUnknown,
                    \'change\', SUM(countingProcessItem.`quantity`) - IFNULL(stockInUnknownStockLocation.`quantity`, 0)
                ),
                NOW()
            FROM `pickware_erp_stocktaking_stocktake_snapshot_item` snapshotCountingProcessItem
            LEFT JOIN `pickware_erp_stocktaking_stocktake_counting_process_item` countingProcessItem
            ...
            LEFT JOIN `pickware_erp_stock` stockInUnknownStockLocation
                ...
                AND stockInUnknownStockLocation.`warehouse_id` = stocktake.`warehouse_id`
                AND stockInUnknownStockLocation.`location_type_technical_name` = "warehouse"
            JOIN (SELECT @rowNumber := :maxRowNumber) r

            WHERE countingProcess.`stocktake_id` = :stocktakeId
            AND snapshotCountingProcessItem.`total_stock_difference` <> 0
            AND countingProcessItem.`product_version_id` = :liveVersionId
            -- Only consider counting processes in the unknown stock location
            AND (countingProcess.`bin_location_id` IS NULL AND countingProcess.`bin_location_snapshot` IS NULL)
            -- "stockInUnknownStockLocation" is the same for all rows of a product. We add this group-by so we can select it above
            GROUP BY countingProcessItem.`product_id`, stockInUnknownStockLocation.`quantity`
            HAVING (SUM(countingProcessItem.`quantity`) - IFNULL(stockInUnknownStockLocation.`quantity`, 0)) <> 0',
            [
