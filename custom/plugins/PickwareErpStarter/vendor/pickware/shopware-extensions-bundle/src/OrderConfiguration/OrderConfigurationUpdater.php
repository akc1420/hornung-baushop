...
/**
 * @deprecated next-major will be removed
 */
class OrderConfigurationUpdater implements EventSubscriberInterface
{
    ...
    /**
     * Env variable from the container to skip this update (entity creation and updates) all together. We add this to
     * avoid the risk of performance issues or deadlocks if and only if we know for sure (in a controlled environment)
     * that this deprecated OrderConfigurationUpdater is not used.
     */
    private bool $skip;
    ...
    /**
     * We limit the OrderConfiguration existence to the live version. We needed to make this restriction to avoid
     * deadlocks in a recent feature development. We know that we _actually_ want all order versions to have an order
     * configuration. But the usages are deprecated (and not used anymore) so we ignore this until 6.5 when the whole
     * class is removed once and for all.
     *
     * @param String[] $orderIds
     */
