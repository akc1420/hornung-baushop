...
use Crsw\CleverReachOfficial\Core\Infrastructure\ORM\Interfaces\QueueItemRepository;
...
use Crsw\CleverReachOfficial\Core\Infrastructure\TaskExecution\Events\BeforeQueueStatusChangeEvent;
use Crsw\CleverReachOfficial\Core\Infrastructure\TaskExecution\Events\QueueStatusChangedEvent;
...
class QueueService
{
     ...
     * @param int $priority
     *
     * @return \Crsw\CleverReachOfficial\Core\Infrastructure\TaskExecution\QueueItem Created queue item.
     *
     * @throws \Crsw\CleverReachOfficial\Core\Infrastructure\TaskExecution\Exceptions\QueueStorageUnavailableException When queue storage fails to save the item.
     */
...
/**
    ...
    {
        $filter = new QueryFilter();
        $lastUpdateTimeFilter = $this->getTimeProvider()->getCurrentLocalTime()->getTimestamp() - Task::MAX_INACTIVITY_PERIOD;
        /** @noinspection PhpUnhandledExceptionInspection */
        $filter->where('status', '=', QueueItem::IN_PROGRESS);
     ...
     *
     * @param \Crsw\CleverReachOfficial\Core\Infrastructure\TaskExecution\QueueItem $queueItem Queue item with is about to change status.
