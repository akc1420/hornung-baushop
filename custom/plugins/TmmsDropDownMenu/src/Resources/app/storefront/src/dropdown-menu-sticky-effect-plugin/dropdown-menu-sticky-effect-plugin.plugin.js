...
export default class DropdownMenuStickyEffectPlugin extends Plugin {
    static options = {
        cloneDropdownMenuMainNavigationStickyClass: 'main-navigation-dropdown-sticky',
        positionDropdownMenuStickyEffectIsActive: 120,
        notActiveViewportsDropdownMenuStickyEffectIsActiveString: "'XS', 'SM', 'MD'",
        dropdownMenuMultiLineOpenLastChildToLeft: false,
        dropdownMenuNumberMainNavigationMenuItemsOpenToLeft: 1,
        dropdownMenuMinimumNumberMainNavigationMenuItemsOpenToLeft: 3,
        elementSelector: "[data-tmms-dropdown-menu-sticky-effect]"
    }
    ...
    init() {
        ...
        if(this.options.dropdownMenuMultiLineOpenLastChildToLeft && this.dropdownMenuShouldBeActive()) {
            ...
            this.setDropdownMenuRight("."+this.options.cloneDropdownMenuMainNavigationStickyClass);
        }
    }

    createElement(){
        this._dropdownMenuMainNavigationClone = this.el.cloneNode(true);
        this._dropdownMenuMainNavigationClone.classList.add(this.options.cloneDropdownMenuMainNavigationStickyClass);

        DomAccess.querySelector(this._dropdownMenuMainNavigationClone, '.main-navigation').removeAttribute('id');

        if((document.querySelectorAll('.main-navigation-wrapper.'+this.options.cloneDropdownMenuMainNavigationStickyClass)).length === 0) {
            document.body.appendChild(this._dropdownMenuMainNavigationClone);
        }
    }
    ...
    onScroll(){
        ...
        if(scrollPosition > this.options.positionDropdownMenuStickyEffectIsActive){
            if(!this._dropdownMenuMainNavigationClone.classList.contains('is--active')) {
                this._dropdownMenuMainNavigationClone.classList.add('is--active');
            }
        } else {
            this._dropdownMenuMainNavigationClone.classList.remove('is--active');
        }
    }
    ...
    pluginShouldBeActive(){
        if((this.options.notActiveViewportsDropdownMenuStickyEffectIsActiveString).includes(ViewportDetection.getCurrentViewport())){
            return false;
        }
    ...
    destroy(){
        this._dropdownMenuMainNavigationClone.remove();
