...
class Migration1643985460Analytics extends MigrationStep
{
    ...
    public function update(Connection $connection): void
    {
        ...
        $connection->executeUpdate(
            ...
            ['hasBoth' => $hasBoth, 'stats' => $statsBoth], ['stats' => Connection::PARAM_STR_ARRAY]
        );

        $connection->executeUpdate(
            'UPDATE `cbax_analytics_config` SET `parameter` = :hasTable WHERE `name` IN (:stats);',
            ['hasTable' => $hasTable, 'stats' => $statsOnlyTable], ['stats' => Connection::PARAM_STR_ARRAY]
        );
        ...
        $allStats = $connection->executeQuery('SELECT `name`, `position` FROM `cbax_analytics_config` WHERE 1;')->fetchAll();
        ...
        $connection->executeUpdate("UPDATE `cbax_analytics_config`
