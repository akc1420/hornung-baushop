...
class Migration1614589470Analytics extends MigrationStep
{
    ...
    public function update(Connection $connection): void
    {
        if ($this->tableExist('cbax_analytics_config', $connection))
        {
            ...
            {
                $connection->executeUpdate("ALTER TABLE `cbax_analytics_config` ADD `plugin` varchar(255) DEFAULT NULL AFTER `active`;");
            }
        }
        ...
        if ($this->tableExist('cbax_analytics_groups_config', $connection))
        {
            ...
            {
                $connection->executeUpdate("ALTER TABLE `cbax_analytics_groups_config` ADD `plugin` varchar(255) DEFAULT NULL AFTER `active`;");
            }
        }
    }
