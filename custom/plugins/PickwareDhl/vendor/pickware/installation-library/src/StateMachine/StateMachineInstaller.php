...
class StateMachineInstaller
{
     ...
     */
    public function ensureStateMachine(StateMachine $stateMachine, ?Context $context = null): void
    {
        if (!$context) {
            trigger_error(
        ...
        if ($this->entityManager) {
            $this->ensureStateMachineWithEntityManager($stateMachine, $context);
     ...
     * @param Context|null $context @deprecated next-major parameter will be non-optional
     */
    public function removeStateMachine(StateMachine $stateMachine, ?Context $context = null): void
    {
        if (!$context) {
            trigger_error(
                sprintf('Not passing %s parameter is deprecated in %s.', Context::class, __METHOD__),
    ...
    private function ensureStateMachineWithEntityManager(StateMachine $stateMachine, Context $context): void
    {
        ...
        foreach ($stateMachine->getStates() as $state) {
            $stateMachineStateId = $this->ensureStateMachineStateWithEntityManager($state, $stateMachineId, $context);
    ...
    private function ensureStateMachineStateWithEntityManager(
